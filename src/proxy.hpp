...
namespace reverse
{
    ...
    public:
        ...
        auto close() -> void
        {
            LOG_INFO(quill::get_logger(), "{}: Shutdown", id_);
    ...
    private:
        template <typename App> auto run(uint16_t timeout, std::string keyfile, std::string certfile) -> void
        {
            ...
            auto const on_message = [this, logger, timeout](auto* ws, std::string_view message, uWS::OpCode opcode) {
            ...
            auto const on_open = [this, logger](auto* ws) {
                ...
                {
                    LOG_ERROR_NOFN(logger, "{}: Connection to node failed @ {}:{}", id_, node_url_, node_port_);
                    ws->close();
                    return;
                }

                LOG_INFO_NOFN(logger, "{}: Connected to node @ {}:{}", id_, node_url_, node_port_);
            };
